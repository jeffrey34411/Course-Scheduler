...
import com.example.collegescheduler2.AddExam;
import com.example.collegescheduler2.Home;
import com.example.collegescheduler2.ListOfCourse;
import com.example.collegescheduler2.ListOfExam;
...
public class ExamAdapter extends RecyclerView.Adapter<ExamAdapter.ViewHolder> {
    private List<ExamModel> todoList;
    private ListOfExam activity;
    public static ExamModel clickedItem = null;

    public ExamAdapter(ListOfExam activity) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        final ExamModel item = Home.examArray.get(position);
        holder.task.setText(item.getName() + " - " + item.getTime() + " - " + item.getLocation());
        ...
        holder.task.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    item.setStatus(true);
                    Home.examArray.remove(item);
                    notifyItemRemoved(position);
                    notifyItemRangeChanged(position, Home.examArray.size());
                } else {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            if (position != RecyclerView.NO_POSITION) {
                clickedItem = Home.examArray.get(position);
                Home.examArray.remove(clickedItem);
                Context context = view.getContext();
                context.startActivity(new Intent(context, AddExam.class));
            }
        }
    }
}